...
def store_data_as_list_in_json_file_in_data_folder(data_as_list, file_name):
    '''
    ...
    '''
     # open/create the JSON file that the data will be written to
    with open( '/Users/andreas/Documents/GitHub/deepRCT/first_deepRCT_Andreas/data' + '/' + file_name + '.json', 'w+') as json_file:
...
def generate_deepRCT_data_and_store(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,num_patients_for_trial, 
num_patients_for_validation,num_patients_for_test,num_baseline_intervals, num_testing_intervals, time_scale_conversion):
    '''
        Inputs:
            ...
            (float) - the mean of the normally distributed placebo efficacy distribution

            8) palcebo_effect_sigma:

            (float) - the standard deviation of the normally distributed palcebo efficay distribution 

            9) num_patients_for_trial:

            (int) - number of patients for the trial dataset

            10) num_patients_for_validation:

            (int) - the number of patients the validation dataset

            11) num_patients_for_test:

            (int) - the number of patients for the test dataset

            12) num_baseline_intervals:

            (int) - the number of weeks for the baseline intervals

            13) num_testing_intervals:

            (int) - the number of weeks for the testing intervals

            14) time_scale_conversion:
    ...
    '''
    #2D numpy array that holds the seizure frequencies for the validation dataset
    seizure_frequencies_validation = np.zeros((num_patients_for_validation,num_baseline_intervals + num_testing_intervals))

    #2D numpy array that holds the seizure frequencies for the validation dataset 
    seizure_frequencies_trial = np.zeros((num_patients_for_trial,num_baseline_intervals + num_testing_intervals))

    #2D numpy array that holds the seizure frequencies for the validation dataset 
    seizure_frequencies_test = np.zeros((num_patients_for_test,num_baseline_intervals + num_testing_intervals))

    #Three 1D numpy arrays that hold ones and zeros. A one if the patient is receiving a drug a zero if it just gets the placebo.
    seizure_frequencies_validation_bool = np.zeros(num_patients_for_validation)
    seizure_frequencies_test_bool = np.zeros(num_patients_for_test)
    seizure_frequencies_trial_bool = np.zeros(num_patients_for_trial)

    #declare two variable to make sure data has half patients that are placebos and half that are getting drugs
    drug_patients  = 0
    placebo_patients  = 0

    #generate num_patients_for_trial patients for deepRCT
    for patients in range(num_patients_for_trial):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,time_scale_conversion, num_baseline_intervals,num_baseline_intervals + num_testing_intervals,4)
        drug = np.random.randint(0,2)
        if(drug == 1 and drug_patients < num_patients_for_trial/2):
            seizure_frequencies_trial[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
            seizure_frequencies_trial_bool[patients] = 1
            drug_patients = drug_patients + 1
        elif(placebo_patients >= num_patients_for_trial/2):
           seizure_frequencies_trial[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
           seizure_frequencies_trial_bool[patients] = 1
           drug_patients = drug_patients  + 1
        else:
            seizure_frequencies_trial[patients] = apply_effect(placebo_effect_mu,placebo_effect_sigma,temp_patient_diary,num_baseline_intervals,num_testing_intervals)
            seizure_frequencies_trial_bool[patients] = 0
            placebo_patients = placebo_patients + 1

    print(drug_patients)
    print(placebo_patients)

    #declare two variable to make sure data has half patients that are placebos and half that are getting drugs
    drug_patients = 0
    placebo_patients = 0
    #generate num_patients_for_validation patients in the validation data set for deepRCT
    for patients in range(num_patients_for_validation):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,time_scale_conversion, num_baseline_intervals, num_baseline_intervals + num_testing_intervals,4)
        drug = np.random.randint(0,2)
        if(drug == 1 and drug_patients < num_patients_for_validation/2):
            seizure_frequencies_validation[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
            seizure_frequencies_validation_bool[patients] = 1
            drug_patients = drug_patients + 1
        elif(placebo_patients >= num_patients_for_validation/2):
           seizure_frequencies_validation[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
           seizure_frequencies_validation_bool[patients] = 1
           drug_patients = drug_patients  + 1
        else:
            seizure_frequencies_validation[patients] = apply_effect(placebo_effect_mu,placebo_effect_sigma,temp_patient_diary,num_baseline_intervals,num_testing_intervals)
            seizure_frequencies_validation_bool[patients] = 0
            placebo_patients = placebo_patients + 1

    #declare two variable to make sure data has half patients that are placebos and half that are getting drugs
    drug_patients = 0
    placebo_patients = 0
    #generate num_patients_for_test patients in the test data set for deepRCT
    for patients in range(num_patients_for_test):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,time_scale_conversion,num_baseline_intervals, num_baseline_intervals + num_testing_intervals,4)
        drug = np.random.randint(0,2)
        if(drug == 1 and drug_patients < num_patients_for_test/2):
            seizure_frequencies_test[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
            seizure_frequencies_test_bool[patients] = 1
            drug_patients = drug_patients + 1
        elif(placebo_patients >= num_patients_for_test/2):
           seizure_frequencies_test[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, num_baseline_intervals, num_testing_intervals)
           seizure_frequencies_test_bool[patients] = 1
           drug_patients = drug_patients  + 1
        else:
             seizure_frequencies_test[patients] = apply_effect(placebo_effect_mu,placebo_effect_sigma,temp_patient_diary,num_baseline_intervals,num_testing_intervals)
             seizure_frequencies_test_bool[patients] = 0
             placebo_patients = placebo_patients + 1

    #store the data from the 2D numpy arrays and the 1D numpy arrays in a new folder called data
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_trial.tolist(),'seizure_frequencies_trial')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_validation.tolist(),'seizure_frequencies_validation')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test.tolist(),'seizure_frequencies_test')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_trial_bool.tolist(), 'seizure_frequencies_trial_bool')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_validation_bool.tolist(), 'seizure_frequencies_validation_bool')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test_bool.tolist(), 'seizure_frequencies_test_bool')

def generate_alltrue(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,num_patients_for_test,num_baseline_intervals, num_testing_intervals, time_scale_conversion):
    '''
        Inputs:
    ...
    #store the data from the 2D numpy arrays and the 1D numpy arrays in a new folder called data
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test.tolist(),'seizure_frequencies_test_all_pos')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test_bool.tolist(), 'seizure_frequencies_test_bool_all_pos')

def generate_allneg(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,num_patients_for_test,num_baseline_intervals, num_testing_intervals, time_scale_conversion):
    '''
        Inputs:
    ...
    #store the data from the 2D numpy arrays and the 1D numpy arrays in a new folder called data
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test.tolist(),'seizure_frequencies_test_all_neg')
    store_data_as_list_in_json_file_in_data_folder(seizure_frequencies_test_bool.tolist(), 'seizure_frequencies_test_bool_all_neg')
...
if (__name__ == '__main__'):
    ...
    '''
    #Generating 10000 patients for deepRCT's training set, 5000 for the validation set and 5000 for test set
    generate_deepRCT_data_and_store(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,10000,5000,5000,num_baseline_intervals,num_testing_intervals,7)
    generate_allneg(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,5000,num_baseline_intervals,num_testing_intervals,7)
    generate_alltrue(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,5000,num_baseline_intervals,num_testing_intervals,7)
