...
def generate_deepRCT_data_and_store(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,num_patietns_for_trial, 
num_patients_for_validation,num_patients_for_test):
    '''
    '''
    seizure_frequencies_validation = np.zeros((5000,20))
    seizure_frequencies_trial = np.zeros((10000,20))
    seizure_frequencies_test = np.zeros((5000,20))

    #declare two variable to make sure data is randomized and about half patients are placebos and half are getting drugs
    drug_patients  = 0
    placebo_patients  = 0
    #generate 10000 patients for deepRCT
    for patients in range(10000):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,7,8,20,4)
        drug = np.random.randint(0,1)
        if(drug == 1 and drug_patients < 5000):
            seizure_frequencies_trial[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
            drug_patients = drug_patients + 1
        elif(placebo_patients >= 5000):
           seizure_frequencies_trial[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
           drug_patients = drug_patients  + 1
        else:
            seizure_frequencies_trial[patients] = temp_patient_diary
            placebo_patients = placebo_patients + 1

    #declare two variable to make sure data is randomized and about half patients are placebos and half are getting drugs
    drug_patients = 0
    placebo_patients = 0
    #generate 5000 patients in the validation data set for deepRCT
    for patients in range(5000):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,7,8,20,4)
        drug = np.random.randint(0,1)
        if(drug == 1 and drug_patients < 2500):
            seizure_frequencies_validation[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
            drug_patients = drug_patients + 1
        elif(placebo_patients >= 2500):
           seizure_frequencies_validation[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
           drug_patients = drug_patients  + 1
        else:
            seizure_frequencies_validation[patients] = temp_patient_diary
            placebo_patients = placebo_patients + 1

    #declare two variable to make sure data is randomized and about half patients are placebos and half are getting drugs
    drug_patients = 0
    placebo_patients = 0
    #generate 5000 patients in the test data set for deepRCT
    for patients in range(5000):
        temp_patient_diary =  generate_patient(shape,scale,alpha,beta,7,8,20,4)
        drug = np.random.randint(0,1)
        if(drug == 1 and drug_patients < 2500):
            seizure_frequencies_test[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
            drug_patients = drug_patients + 1
        elif(placebo_patients >= 2500):
           seizure_frequencies_test[patients] = apply_effect(drug_effect_mu, drug_effect_sigma, temp_patient_diary, 8, 12)
           drug_patients = drug_patients  + 1
        else:
            seizure_frequencies_test[patients] = temp_patient_diary
            placebo_patients = placebo_patients + 1

    store_data_as_list_in_json_file(seizure_frequencies_trial.tolist(),'seizure_frequencies_trial')
    store_data_as_list_in_json_file(seizure_frequencies_validation.tolist(),'seizure_frequencies_validation')
    store_data_as_list_in_json_file(seizure_frequencies_test.tolist(),'seizure_frequencies_test')
...
if (__name__ == '__main__'):
    ...
    generate_deepRCT_data_and_store(shape,scale,alpha,beta,drug_effect_mu,drug_effect_sigma,placebo_effect_mu,placebo_effect_sigma,10000,5000,5000)
